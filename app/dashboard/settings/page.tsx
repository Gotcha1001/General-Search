...
/** Renders the matching animated scene for an option, or null if it's a static one. */
function AnimatedScene({ variant }: { variant?: "cyberVoid" | "matrixCore" }) {
  if (variant === "cyberVoid") return <CyberVoidBackground />;
  if (variant === "matrixCore") return <MatrixCoreBackground />;
  return null;
}
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="p-6 rounded-2xl border border-white/30 bg-black/30 backdrop-blur-sm shadow-sm mb-6">
        ...
        <div className="relative w-full h-36 rounded-xl overflow-hidden mb-6 border border-white/30">
          ...
          {activeBg.animated ? (
            <AnimatedScene variant={activeBg.animated} />
          ) : activeBg.src ? (
            <img
              src={activeBg.src}
              alt=""
          ...
          {BACKGROUNDS.map((bg) => {
            ...
            return (
              ...
              >
                ...
                {bg.animated ? (
                  <AnimatedScene variant={bg.animated} />
                ) : bg.thumbnail ? (
                  <img
                    src={bg.thumbnail}
                    alt={bg.label}
